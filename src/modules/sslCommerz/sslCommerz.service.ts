...
const sslCommerzPaymentInit = async (payload: ISSLCommerz) => {
	try {
		const data = {
		...
		console.log("Payment Error accord", error);
		throw new AppError(httpStatus.BAD_REQUEST, "Payment failed. Please try again.");
	}
};

export const SSLCommerzServices = {
	sslCommerzPaymentInit,
};
